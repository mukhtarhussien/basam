...
export default async function Home() {
  ...
  const sectionMap = new Map(
    settings.homeSections.map((s) => [s.key, s])
  );

  const isVisible = (key: string) =>
    sectionMap.get(key)?.visible ?? true;
  ...
  return (
    <main className="min-h-screen bg-background text-foreground p-4 flex flex-col gap-5">
      ...
      <span hidden>
        7OyhQSTMAqQsUIQaRnvgnNBbE8zC1e4xK1AzmCljciOi7O3r
      </span>
        ...
        slides={
          ads.length > 0
            ? ads.slice(0, 4).map((ad) => ({
                title: ad.title,
                subtitle:
                  ad.description ?? `مدة العرض: ${ad.duration}`,
                image: ad.image,
                cta: "شوف العرض",
                href: "/ads",
              }))
